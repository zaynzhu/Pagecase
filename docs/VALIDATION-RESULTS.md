...
0.6 完成 Safari 按需收纳、Safari 合集、持久化浏览器来源，以及 Chrome／Safari 明确分区。导入也增加只读分区预览和来源选择，确认前不写入。Chrome 实时页进一步增加“全部 / 需保存 / 已收纳”状态筛选，使保存后手动关闭的下一步更明确。Safari 只在用户点击时读取最前方窗口；没有插件、后台轮询或常驻辅助进程。Chrome 的实时镜像、快照覆盖和安全恢复边界保持不变。
...
| 指标 | 实测 | 门槛 |
|---|---:|---:|
| 应用物理内存足迹 | 44MB | 目标 ≤80MB，上限 ≤100MB |
| 应用 RSS（诊断值） | 60 秒后约 101.9MB | 记录共享框架与图形映射，不单独作为内存压力结论 |
| 应用 CPU（10 次稳定采样平均） | 约 0.57%，最终 0.0% | ≤1% |
...
本轮同机 500 页 Release 进程在空闲 60 秒后物理内存足迹为 44MB，与上一轮持平；RSS 由约 113.3MB 降至约 101.9MB。RSS 仍受 macOS 共享框架和图形映射影响，因此继续以 `footprint` 作为实际内存压力结论，并保留 RSS 供后续同条件对照。
