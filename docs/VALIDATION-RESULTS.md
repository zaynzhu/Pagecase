...
| `swift run PagecaseCoreChecks` | 40 项通过 |
| 扩展语法检查 | 通过 |
| 扩展 Node 测试 | 8 项通过 |
| 扩展禁止 API 扫描 | 零命中 |
| 扩展外部网络调用扫描 | 零命中 |
...
| 指标 | 实测 | 门槛 |
|---|---:|---:|
| 应用常驻内存（500 页、90 秒） | 50MB | 目标 ≤80MB，上限 ≤100MB |
| 应用物理内存足迹（500 页、90 秒） | 41MB | ≤100MB |
| 应用空闲 CPU（5 次稳定采样平均） | 0.1% | ≤1% |
...
覆盖核对在 500 页无快照场景中完成整页计算并显示未保存状态；持续空闲 90 秒后仍在内存与 CPU 目标内。
