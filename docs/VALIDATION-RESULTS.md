...
| 指标 | 实测 | 门槛 |
|---|---:|---:|
| 应用物理内存足迹 | 45MB，峰值 47MB | 目标 ≤80MB，上限 ≤100MB |
| 应用 RSS（`ps` 诊断值） | 约 114MB | 记录，不单独作为内存压力结论 |
| 应用 CPU（10 次稳定采样平均） | 0.16%，最终 0.0% | ≤1% |
...
本轮追加同机 500 页隔离 Release 进程测量：空闲采样期间物理内存足迹为 45MB、峰值 47MB，`ps` RSS 约 114MB；10 次稳定 CPU 采样平均 0.16%，最终为 0.0%。上一轮关闭主窗口后的菜单栏驻留物理内存足迹为 46MB、峰值 47MB，10 次 CPU 平均 0.03%。与此前约 40MB 的应用足迹处在同一量级，没有出现异常增长；“可以手动关闭”清单只在视图求值时比较内存中的现有数据，没有增加轮询器、数据库、WebView 或额外常驻进程。
