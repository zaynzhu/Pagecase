...
0.3 版本在纯演示数据下完成单个标签组保存、旧快照兼容、安全、功能、视觉、打包与性能验证。整个自动化过程没有加载 Chrome 扩展、没有注册真实 Native Messaging Host，也没有读取或操作用户当前的 Chrome。
...
| `swift run PagecaseCoreChecks` | 53 项通过 |
| 扩展语法检查 | 通过 |
| 扩展 Node 测试 | 10 项通过 |
...
| 指标 | 实测 | 门槛 |
|---|---:|---:|
| 应用常驻内存（500 页、60 秒） | 约 52MB | 目标 ≤80MB，上限 ≤100MB |
| 应用物理内存足迹（500 页、60 秒） | 39MB | ≤100MB |
| 应用空闲 CPU（6 次稳定采样平均） | 约 0.1% | ≤1% |
| 菜单栏驻留 RSS（关闭窗口 45 秒） | 约 58MB | ≤100MB |
| 菜单栏驻留物理内存足迹 | 42MB | ≤100MB |
| 菜单栏驻留 CPU | 0.0% | ≤1% |
| Bridge 常驻内存 | 约 6.3MB | ≤25MB |
| Bridge 空闲 CPU | 0.0% | ≤1% |
