...
关闭主窗口不会退出页匣；需要时可以从菜单栏重新打开，退出请使用菜单中的“退出页匣”或 `⌘Q`。
...
- 唯一主窗口、菜单栏驻留和搜索聚焦验收
- 快照覆盖、重复网址、标签组语境与保存后即时更新验收
- 500 页空闲 90 秒性能验收

详细结果见 [当前验证结果](docs/VALIDATION-RESULTS.md)。

## Chrome 集成状态

用户已经手动完成扩展加载与本地连接，并确认实时现场正常显示。自动化开发和当前验收仍只使用隔离模拟数据，没有读取、关闭、移动或改动用户现有的 Chrome 页面；真实 Chrome 中的整组恢复仍待手动验证。
